...
// adaptarse a tema claro/oscuro; el glifo y la X toman el degradado de los
// tokens --lockup-stop-* (theme.css): en la paleta ABRAX es el degradado de
// marca; cualquier otra paleta los aplana a su acento y tiñe currentColor
// vía la regla `.abrax-lockup` de App.css (variante monocroma).
//
...
// stop-color es propiedad CSS: vía style acepta var(), con el hex de marca de
// respaldo por si el token no está definido (p. ej. tests fuera de la app).
const STOP_A = { stopColor: "var(--lockup-stop-a, #2FD9FF)" };
const STOP_B = { stopColor: "var(--lockup-stop-b, #8B5CF6)" };
const STOP_C = { stopColor: "var(--lockup-stop-c, #F23DC4)" };
...
const Defs = ({ idPrefix }: { idPrefix: string }) => (
  <defs>
    <linearGradient
      ...
      y2="110"
    >
      <stop offset="0" style={STOP_A} />
      <stop offset="0.55" style={STOP_B} />
      <stop offset="1" style={STOP_C} />
    </linearGradient>
    <linearGradient
      id={`${idPrefix}-x`}
      gradientUnits="userSpaceOnUse"
  ...
  const svg =
    variant === "stacked" ? (
      <svg
        width={width}
        height={height}
        className={`abrax-lockup ${withTagline ? "" : (className ?? "")}`}
        viewBox="0 0 300 232"
        fill="none"
