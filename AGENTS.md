...
ABRAX is an AI-native project: AI coding assistants are first-class contributors here, and this file is their onboarding. The product itself is speech-first and Spanish-first (es-419) — keep both in mind when touching copy or UX.
...
ABRAX (binary `abrax`) is a cross-platform desktop speech-to-text application built with Tauri 2.x (Rust backend + React/TypeScript frontend). It is a friendly fork of [Handy](https://github.com/cjpais/Handy) (MIT) — see `ATTRIBUTION.md` and `UPSTREAM.md`.
...
- `managers/` - Core business logic:
  ...
  - `dictionary.rs` - Living Dictionary: local project index that biases transcription toward your repo's terms
  - `escucha/` - Escucha (read-aloud): system TTS manager + es-419 preprocessor (`preproceso.rs`)
- `prompt_compiler.rs` - Prompt Compiler: template mode structuring + es-419 ambiguity linter (no LLM)
- `user_alerts.rs` - Single alert channel (recent-error registry + typed event + native notification)
- `visualizer.rs` - Spectrum frames (32 log bands) under a strict subscription model (`start_spectrum`/`stop_spectrum`)
...
Abrax supports command-line parameters on all platforms for integration with scripts, window managers, and autostart configurations.
...
| `--transcribe-file <WAV>`          | Transcribe a 16 kHz mono WAV headlessly and exit (no mic, no VAD; model must be installed). With `--model`, `--device-index`, `--json` |
| `--list-models` / `--list-devices` | List available models / compute devices and exit (headless)                                                                            |
...
- **Opening an issue:** Read [`.github/ISSUE_TEMPLATE/`](.github/ISSUE_TEMPLATE/). Blank issues are disabled; pick the right template (`bug_report.md` for bugs). Feature ideas belong in this repository's Discussions, not in issues.
