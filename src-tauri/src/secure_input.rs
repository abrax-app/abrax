...
// `Emitter` solo lo usa la implementación de macOS (evento
// `secure-input-changed`); fuera de macOS ese código no se compila.
#[cfg(target_os = "macos")]
use tauri::Emitter;
...
///
/// ABRAX — port parcial: este es el enganche de la insignia de bandeja y del
/// banner, que se dejaron FUERA a propósito (ver el informe del 26/07). Se
/// conserva idéntico al upstream para que completar el port sea solo quitar
/// este `allow` y llamarlo desde `tray.rs`.
#[allow(dead_code)]
...
mod imp {
    ...
    /// expressed and are reported as uncovered.
    fn activate_fallback(app: &AppHandle) {
        ...
        for (id, binding) in &settings.bindings {
            if id == "cancel" {
                continue; // registered dynamically during recording only
            }
            ...
            let Ok(hotkey) = binding.current_binding.parse::<handy_keys::Hotkey>() else {
                // A stored binding handy-keys itself can't parse shouldn't
                // exist — surface it rather than skipping silently.
                warn!(
            ...
            let Some((carbon_binding, degraded)) = carbon_equivalent(&hotkey) else {
                ...
                fallback.uncovered.push(id.clone());
                continue;
            };
            ...
            match crate::shortcut::tauri_impl::register_shortcut(app, shadow.clone()) {
                Ok(()) => {
                    ...
                    if degraded {
                        fallback.degraded.push(id.clone());
                    } else {
                        fallback.covered.push(id.clone());
                    }
                }
                Err(e) => {
                    warn!(
...
mod imp {
    ...
    impl SecureInputState {
        ...
        // Solo lo consume `tray_warning_active`, que en este port parcial no se
        // llama todavía (la insignia de bandeja quedó fuera).
        #[allow(dead_code)]
        pub fn warning_active(&self) -> bool {
            false
        }
